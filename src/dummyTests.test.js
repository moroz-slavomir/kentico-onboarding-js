...
describe('Promise rejection', () => {
  it('rejects with error - constructor', () => {
    const myPromise = new Promise((resolve, reject) => reject(Error('Ooops...')));

    return myPromise
      .then(() => fail('This should never happen. Catch block should be called immediately'))
      .catch(error => expect(error.message).toBe('Ooops...'));
  });

  it('rejects with error - shorthand syntax', () => {
    return Promise.reject(Error('Ooops...'))
      .then(() => fail('This should never happen. Catch block should be called immediately'))
      .catch(error => expect(error.message).toBe('Ooops...'));
  });

  it('throws and error falls to catch block', () => {
    const chainedPromise = new Promise(resolve => resolve(-1))
      .then((result) => {
    ...
    return chainedPromise
      ...
      .catch(error => expect(error.toString().indexOf('-1') > -1).toBeTruthy());
  });
});
